...
import schema from '../validation/sites/sites.schema.js';
...
router.get('/',(req,res)=>{
    // res.send(users);
    connection.query("SELECT * FROM sites where company_id = (?)",[[req.query.company_id]],(err,rows,fields)=>{
        if(!err){
            console.log(fields);
            res.status(200).send(rows);
        }else{
...
router.post('/update_status',async (req,res,next)=>{
    try{
        connection.query("UPDATE sites SET status = \"completed\" where id = (?)",[[req.body.id]],(err,rows,fields)=>{
            if(!err){
                var response={}
                response['message']="Status updates successfully";
...
router.post('/', async (req,res,next)=>{
    try{
        ...
        if(response!==true){
            ...
            connection.query("INSERT INTO sites (company_id,name,address,street,mobile,details,status) VALUES (?)",[[req.query.company_id,req.body.name,req.body.address,req.body.street,req.body.mobile,req.body.details,"in_progresss"]],(err,rows,fields)=>{
                        if(!err){
                            var response={};
